...
@Test
public class TC2_VerifyArticleWithLogs_WithScreenshot

{
	...
	public void VerifyArticle()
	{
	    ...
	    String actualtitle=	new String("articles.ArtileTille()");
        String expectedsring = new String("latestnews.Selectarticle()");
		
		try { Assert.assertEquals(true, articles.ArtileTille().isDisplayed()); }
		  catch (Throwable e) {  }
		
		try { expectedsring.contains(actualtitle); }
		  catch (Throwable e) { }
		  ...
		  try { Assert.assertEquals(true, articles.ArticleDescription().isDisplayed());
		  } catch (Throwable e) {  }
		  
		  try {
		  Assert.assertEquals(true, articles.ArticleHeroImage().isDisplayed());
		  } catch (Throwable e) {  }
		 
		  logger.log(LogStatus.INFO, logger.addScreenCapture(Helper.captureScreenshots(driver, "screenshotvalidations")));
		  logger.log(LogStatus.PASS, "Article is present");
	}
